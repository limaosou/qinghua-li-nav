...
本项目的站点品牌叫 **青花狸导航**。`青花狸` 是站长 **李清华**（拼音 *Lǐ Qīnghuá*）名字的**谐音萌化**写法——`李清华` 与 `青花狸` 读音完全相同（`lǐ qīng huá` ↔ `qīng huā lí`），取「蓝白小狸」之意作站点吉祥物与品牌名。

因此仓库 / 站点里多处出现的 `qinghua`、`liqinghua.com` 都源自站长本名：

- GitHub 仓库：`limaosou/qinghua-nav`
- 演示域名：[www.liqinghua.com](https://www.liqinghua.com)（即「李清华.com」）
- LICENSE 版权方：`qinghua`

开源后这些字样会随仓库一并公开。若你更想用品牌名 `青花狸` 而非真名，告诉我即可统一替换。
